# -*- coding: utf-8 -*-
"""
A fully-functional HTTP/2 server using asyncio. Copied from:

https://github.com/python-hyper/hyper-h2/blob/master/examples/asyncio/asyncio-server.py

Try with: 

curl --insecure --http2 https://localhost:8443
...
if False:
    ...
    async def run_server():
        async with trio.open_nursery() as nursery:
             server = await create_server(nursery, H2Protocol, '127.0.0.1', 8443, ssl=ssl_context)
             print('Serving on {}'.format(server.sockets[0].getsockname()))
